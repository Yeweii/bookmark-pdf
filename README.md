...
- ✅ **获取后弹框保存到指定文件夹**（书名自动 sanitize）
- ✅ **书签源可为空**（在线获取后直接挂载，无需 TXT/MD 文件）
- ✅ **书签文本编辑区**（直接编辑 / 从 PDF 读取 / 粘贴外部文本）
- ✅ **书签文本工具集**（页码偏移 / 归一化 / 裁剪 / 去重 / Trim / 展平 / 按页排序）
- ✅ Tkinter GUI，所见即所得操作
- ✅ 树形预览，页码异常实时标红
- ✅ 后台线程挂载，进度条 + 日志
- ✅ 三种输出策略：新文件 / 原位覆盖 / 追加
...
0. 在线获取（SSID → 一键拉取目录）
1. 选择文件（书签源 + PDF）
2. 书签文本（可编辑）         ← 新增：直接编辑 / 从 PDF 读取 / 粘贴外部文本
3. 解析规则（模板 + 自定义正则）
4. 预览（树形结构，页码异常标红）
5. 输出选项（新文件 / 覆盖 / 页码 -1）
6. 进度与日志
...
新增的「2. 书签文本（可编辑）」section 提供三种书签输入方式：

#### 场景 A：从 PDF 读取书签 → 编辑 → 挂回

1. 在「1. 选择文件」先选择 PDF
2. 点击「📥 从 PDF 读取书签」，现有 outline 转 `indent-dot` 格式填入编辑区
3. 在编辑区直接修改（500ms 自动解析，刷新预览）
4. 点击「⚙ 执行挂载」

#### 场景 B：外部粘贴书签 → 编辑 → 挂载

1. 从微信 / 网页 / PDF 阅读器复制书签文本（如 `第一章 ...... 1`）
2. 粘贴到编辑区
3. 自动解析 → 预览显示 → 点击「⚙ 执行挂载」

#### 场景 C：原有 TXT/MD 文件输入（仍可用）
...
| 按钮 | 行为 |
|------|------|
| 📥 从 PDF 读取书签 | 读取 `_pdf_path` 的 outline → 填入编辑区 |
| 🔄 解析文本 | 用 `indent-dot` 规则解析当前文本 → 刷新预览 |
| **页码 +/- `[N]` 应用** | 所有页码批量偏移 N（None 不变） |
| 🔧 工具 ▼ | 弹出工具窗：归一化 / 裁剪 / 去重 / 移除异常 / Trim / 展平 / 按页排序 |
| 📋 清空 | 清空文本、预览、节点缓存 |
| 💾 导出 TXT | 把当前文本保存为 `.txt`（写到「1. 选择文件」的书签源路径，或弹窗另存为） |

### 🔧 工具弹窗（v1.4）

点「🔧 工具 ▼」打开工具弹窗，按分组列出 8 个批量编辑工具：

| 分组 | 工具 | 行为 |
|------|------|------|
| 页码 | 归一化 | 从 1 开始按 DFS 顺序重新编号 |
| 页码 | 裁剪到最大页 | 超过指定页的设为 None |
| 清理 | 去除重复条目 | (title, page) 重复的去重，保留首次 |
| 清理 | 移除异常页码条目 | 移除 page=None 的条目 |
| 文本 | 去除标题首尾空白 | Trim 所有标题 |
| 树形 | 展平 | 移除所有层级，DFS 展平到顶层 |
| 树形 | 按页码升序 | 升序排列（None 末尾） |
| 树形 | 按页码降序 | 降序排列（None 末尾） |

所有工具基于 `bookmark_pdf.transforms` 模块（纯函数，可独立使用）。

### 在线获取流程

1. 在「0. 在线获取」输入框填入 SSID（如 `13284383`）
2. 点击「🌐 获取书签」，后台从 `api.pdfshuwu.com` 拉取目录
3. 成功后自动填入「4. 预览」与「2. 书签文本」区
4. 选择对应 PDF → 点击「⚙ 执行挂载」
5. 完成后**自动保存** `<pdf>_bookmarks.txt`（与 PDF 同目录）

#### v1.3：获取后保存到指定文件夹

在线获取成功后，会弹框询问「是否保存书签？」：

- **是** → 弹出文件夹选择对话框 → 保存为 `<书名>_bookmarks.txt`（书名自动 sanitize 非法字符）
- **否** → 不保存，继续（仍可走「💾 导出 TXT」手动导出）
- **取消文件夹选择** → 不保存，无错误

> 书名为空时回退到 SSID；都为空时使用 `bookmarks`。源文件名已含 `bookmarks` 时不会重复追加后缀。

#### v1.3：书签源可为空（在线获取场景）

获取成功后**无需在「1. 选择文件」选择书签源**，直接选 PDF 即可点「⚙ 执行挂载」。
「0. 在线获取」区有提示标签说明此流程。
...
- v1.0：核心解析 + 挂载 + GUI + pip 安装 + PyInstaller 打包
- v1.1：在线书签获取（SSID）+ 挂载后自动保存 TXT
- v1.2：书签文本编辑区（直接编辑 / 从 PDF 读取 / 粘贴外部文本）
- v1.3：在线获取后弹框保存到指定文件夹 + 书签源可为空
- v1.4：书签文本工具集（页码偏移 / 归一化 / 裁剪 / 去重 / Trim / 展平 / 按页排序）
- **v1.5（当前）**：合并发布 v1.2 + v1.3 + v1.4，重新打包 macOS `.app`
- v1.6：书签模板保存/加载（用户预设）
- v1.7：批量处理（多 PDF 队列）
- v2.0：CI 自动构建多平台 release（GitHub Actions）

> 详细变更见 [`CHANGELOG.md`](CHANGELOG.md) 与 [`docs/RELEASE-v1.5.0.md`](docs/RELEASE-v1.5.0.md)
